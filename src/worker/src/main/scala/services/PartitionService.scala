...
class PartitionService(
  ...
  // 임시 파일명 규칙
  private def getChunkFileName(destWorkerId: Int, uniqueId: String): String =
    s"$temp/chunk_partition_for_worker_${destWorkerId}_${uniqueId}.dat"
  ...
  def partitionRecords(
    ...
    try {
      sortedRecords.foreach { record =>
        ...
        val filePath = getChunkFileName(destWorkerId, uniqueId)
